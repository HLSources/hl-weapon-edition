...
void SpectatorPanel::Initialize()
{
	...
	m_ExtraInfo->setFgColor( vgui::Scheme::sc_primary1 );
	...
	m_CurrentTime->setFgColor( vgui::Scheme::sc_primary1 );
	...
	for ( int j= 0; j < TEAM_NUMBER; j++ )
	{
		...
		m_TeamScores[j]->setFgColor( vgui::Scheme::sc_primary1 );
	...
	m_TopHelpLabel = new Label ("FIRE - spawn             DUCK - change mode\nJUMP - change target   USE - toggle tactical map",
		XRES(25), YRES(0), XRES(550), YRES(40));
	m_TopHelpLabel->setParent(m_TopBorder);
	m_TopHelpLabel->setPaintBackgroundEnabled(false);
	m_TopHelpLabel->setContentAlignment( vgui::Label::a_northwest );
	m_TopHelpLabel->setFgColor( vgui::Scheme::sc_primary1 );
	m_TopHelpLabel->setFont( pSchemes->getFont(hSmallScheme) );
